...
use app\utils\wechat\DataTransform;
...
class RePayNotifyController extends Controller
{
    ...
    private function hyOrderNotify($res)
    {
        ...
        if ($order->save()) {
            ...
            $vip_end_time = intval($user->vip_end_time);
            if ($vip_end_time < strval(time())) {
                $vip_end_time = strval(time()); //当前时间开始
            }
            $vip_time = intval($order->vip_time);
            $user->vip_end_time = intval(($vip_end_time + $vip_time));
            $user->save();
            echo '<xml><return_code><![CDATA[SUCCESS]]></return_code><return_msg><![CDATA[OK]]></return_msg></xml>';
            return;
        } else {
