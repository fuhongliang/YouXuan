...
<head>
    ...
    <!--新图标 -->
    <link href="//at.alicdn.com/t/font_1449456_qjci5oe4b88.css" rel="stylesheet">
    ...
    <link href="<?= Yii::$app->request->baseUrl ?>/statics/css/color.css?version=<?= $version ?>" rel="stylesheet">
    <link href="<?= Yii::$app->request->baseUrl ?>/statics/css/navbar.css?version=<?= $version ?>" rel="stylesheet">
...
?>
<div class="sidebar <?= $current_menu && !empty($current_menu['children']) ? 'sidebar-sub' : null ?>">
    <div class="sidebar-1">
        <div class="logo">
            <img src="<?= Yii::$app->request->baseUrl ?>/statics/images/logo.png" alt="">
        </div>
        <div>
    ...
    <?php if ($current_menu && !empty($current_menu['children'])): ?>
        <div class="sidebar-2">
            <div class="current-menu-name"><?= $current_menu['name'] ?></div>
            <div class="sidebar-content">
                <?php foreach ($current_menu['children'] as $item): ?>
                    <?php if (isset($item['children']) && is_array($item['children']) && !empty($item['children']) > 0): ?>
                        <a class="nav-item parent-nav-item <?= activeMenu($item, $route) ?>"
                           href="javascript:">
                            <span class="nav-pointer iconfont iconright-s"></span>
                            <span><?= $item['name'] ?></span>
                        </a>
...
<div class="main">
    <div class="main-header">
        <nav class="navbar navbar-expand navbar-light topbar mb-4">
            ...
            <?php if (!empty($this->params['page_navs'])&&is_array($this->params['page_navs'])): ?>
            <ul class="nav nav-tabs-custom">
                <?php foreach ($this->params['page_navs'] as $page_nav): ?>
                    <li class="<?= $page_nav['active'] ? 'active' : null ?>"><a href="<?= $page_nav['url'] ?>"><?= $page_nav['name'] ?></a></li>
                <?php endforeach; ?>
            </ul>
            <?php endif; ?>
            ...
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item dropdown no-arrow mx-1">
                    <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        ...
                        <span class="badge badge-danger badge-counter totalNum" hidden>3+</span>
                    </a>
                    <!-- Dropdown - Alerts -->
                ...
                <li class="nav-item dropdown no-arrow">
                    ...
                    <span class="mr-2 d-none d-lg-inline text-gray-600 small">
                        ...
                        <img class="img-profile rounded-circle" src="<?= Yii::$app->request->baseUrl ?>/statics/images/logo.png"">
                    </a>
                    <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="userDropdown">
                        <a class="dropdown-item" href="#">修改密码</a>
                        <?php if (Yii::$app->user->isGuest == false): ?>
                            <a class="dropdown-item" href="<?= Yii::$app->urlManager->createUrl(['admin/default/index']) ?>">返回系统</a>
                    ...
                    success: function (res) {
                        ...
                        if (res.code == 0) {
                            ...
                            for (var i = 0; i < count; i++) {
                                ...
                                if (order_type == 4) {
                                    var html = "<a target='_blank' class='dropdown-item d-flex align-items-center' data-index='" + res.data[i].id + "' href='" + res.data[i].url + "'><span class=\"font-weight-bold\">" + res.data[i].name + "申请商品上架</span></a>";
                                } else {
                                    if (type == 0) {
                                        var html = "<a target='_blank' class='dropdown-item d-flex align-items-center' data-index='" + res.data[i].id + "' href='" + res.data[i].url + "'><span class=\"font-weight-bold\">" + res.data[i].name + "下了一个订单</span></a>";
                                    } else {
                                        var html = "<a target='_blank' class='dropdown-item d-flex align-items-center' data-index='" + res.data[i].id + "' href='" + res.data[i].url + "'><span class=\"font-weight-bold\">" + res.data[i].name + "一个售后订单</span></a>";
                                    }
                                }
