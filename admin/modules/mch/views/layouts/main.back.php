...
<nav class="header-nav navbar navbar-toggleable-md navbar-light bg-faded">
        ...
        <ul class="navbar-nav">
            <?php foreach ($menu_list as $item): ?>
                <li class="nav-item <?= activeMenu($item, $route) ?>">
                    <a class="nav-link" href="<?= $urlManager->createUrl($item['route']) ?>"><span class="nav-icon iconfont <?= $item['icon'] ?>"></span>
                        <?= $item['name'] ?>
                    </a>
                </li>
            <?php endforeach; ?>
...
<div class="sidebar fl <?= $current_menu && count($current_menu['children']) ? 'sidebar-sub' : null ?>">
