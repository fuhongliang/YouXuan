...
//            ],
    ...
    public function actionIndex()
    {
        ...
        $vip_end_date = empty(\Yii::$app->user->identity->vip_end_time) ? '' : date('Y-m-d',\Yii::$app->user->identity->vip_end_time);
        $user_info = [
            ...
            'vip_end_time' => \Yii::$app->user->identity->vip_end_time,
    ...
    {
        $user_id = empty(\Yii::$app->user->id) ? 2 : \Yii::$app->user->id;
        $list = Address::find()->select('id,name,mobile,province_id,province,city_id,city,district_id,district,detail,is_default,areaCode')->where([
        ...
        foreach ($list as $i => $item) {
            ...
            $list[$i]['tel'] = $item['mobile'];
            $list[$i]['isDefault'] = $item['is_default'] ? true : false;
            $list[$i]['addressDetail'] = $item['detail'];
        }
    ...
    {
        $address = Address::find()->select('id,name,mobile,province_id,province,city_id,city,district_id,district,detail,is_default,areaCode')->where([
            'store_id' => $this->store->id,
    ...
    public function actionAddressSave()
    {
        ...
        if ($app_key == 'h5') {
            ...
            $postData['user_id'] = empty(\Yii::$app->user->id) ? 2 : \Yii::$app->user->id;
            $postData['store_id'] = $this->store->id;
            $res = $addressService->saveAddress($postData);
